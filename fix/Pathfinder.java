...
public class Pathfinder {
	...
	private Tile pathfind(Enemy e){
		char[][] b=new char[board.numRows()][board[0].numCols()];
		...
		frontier.add(e.where());
		boolean solved = false;
		Tile current = null;
		while(frontier.hasNext() && !solved){
			current=frontier.remove();
			if (current==board.getPlayer().where()){
				solved=true;
			} else {
				b[current.getRow()][current.getCol()]='.';
				if (b[current.getRow()+1][current.getCol()]==' '){
					frontier.add(new Node(board.get(current.getRow()+1,current.getCol()), current.traveled,current));
					b[current.getRow()+1][current.getCol()]='?';
				}
				if (b[current.getRow()-1][current.getCol()]==' '){
					frontier.add(new Node(board.get(current.getRow()-1,current.getCol()), current.traveled,current));
					b[current.getRow()-1][current.getCol()]='?';
				}
				if (b[current.getRow()][current.getCol()+1]==' '){
					frontier.add(new Node(board.get(current.getRow(),current.getCol()+1), current.traveled,current));
					b[current.getRow()][current.getCol()+1]='?';
				}
				if (b[current.getRow()][current.getCol()-1]==' '){
					frontier.add(new Node(board.get(current.getRow(),current.getCol()-1), current.traveled,current));
					b[current.getRow()][current.getCol()-1]='?';
